...
class UdemyScraper:
    ...
    def login(self, force: bool = False) -> bool:
        ...
        with SB(uc=True, test=True, locale_code="en") as sb:
            ...
            passwordless_button_selector_1 = "button[type='submit']:contains('Continue with email')"
            ...
            try:
                ...
                if not force and os.path.exists(self.cookies_file):  # use existing cookies if available
                ...
                return self._perform_login(sb, login_url, login_selector, password_selector,
                                           passwordless_button_selector_1, passwordless_button_selector_2,
                                           passwordbased_button_selector)
    
    def _perform_login(self, sb, login_url, login_selector, password_selector,
                      passwordless_button_selector_1, passwordless_button_selector_2,
                      passwordbased_button_selector) -> bool:
        ...
        while current_attempt < max_attempts:
            ...
            try:
                ...
                try:  # try passwordless login first
                    ...
                    print("WARNING: ENTER LOGIN CODE FROM EMAIL AND WAIT 60s FOR AUTO CLICK!")
                    sb.wait(60)
                ...
                print(f"WARNING: LOGIN FAILED (Attempt {current_attempt}/{max_attempts})")
    ...
    def scrape_courses(self) -> Dict[str, str]:
        ...
        with SB(uc=True, test=True, locale_code="en") as sb:
            ...
            courses_list = self._get_all_course_urls(sb, courses_url, course_grid_selector, pages_count)
            
            # Get existing courses to avoid re-scraping
            existing_courses = self._get_existing_courses()
            
            # Scrape details for each course
            courses_details = self._scrape_course_details(sb, courses_list, existing_courses,
                                                         overview_selector, time_selector)
    ...
    def _get_course_metadata(self, sb) -> Tuple[List[str], int, int]:
        ...
        courses_num_details = soup.select("div[class*='pagination-label']")[0].text.strip()
    ...
    def _get_all_course_urls(self, sb, courses_url: str, 
                            course_grid_selector: str, pages_count: int) -> List[str]:
        ...
        for page in range(1, 2):
            ...
            soup = BeautifulSoup(sb.get_page_source(), "lxml")
            course_elements = soup.find_all("h3", attrs={"data-purpose": "course-title-url"})
    ...
    def _scrape_course_details(self, sb, courses_list: List[str], existing_courses: set,
                              overview_selector: str, time_selector: str) -> Dict[str, str]:
        """Scrape details for each course.
    ...
    def format_csv(self) -> None:
        ...
        with (
            ...
            open(self.formatted_courses_file, "w", newline="", encoding="utf-8") as output_file,
        ):
            ...
            try:
                ...
                if course_count > 0:
                    ...
                    print(f"INFO: Total course time: {hours:.1f} hours ({total_minutes} minutes)")
                    print(f"INFO: Average course length: {(total_minutes/course_count):.1f} minutes")
